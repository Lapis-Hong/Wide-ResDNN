...
if [ $# -eq 2 ] ;then
    awk -F '\t' '{v[NR]=$"'$2'";sum+=$"'$2'"} END {avg=sum/NR;for(n in v)sd+=(v[n]-avg)^2;sd=sqrt(sd/NR);printf("f%d\t%f\t%f\n",(("'$2'"-1)),avg,sd)}' $1

elif [ $# -eq 3 ] ;then
    for i in $(seq $2 $3)
    do
        awk -v featureIndex=${i} -F '\t' '{v[NR]=$featureIndex;sum+=$featureIndex} END {avg=sum/NR;for(n=1;n<=NR;n++)sd+=(v[n]-avg)^2;sd=sqrt(sd/NR);printf("f%d\t%f\t%f\n",featureIndex-1,avg,sd)}' $1
    done
